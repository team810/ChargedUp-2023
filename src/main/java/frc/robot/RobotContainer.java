...
public class RobotContainer {
    ...
    new Trigger(OIConstants.DRIVE_GAMEPAD::getAButton).whileTrue(
            new StartEndCommand(
                () -> {m_intake.runIntake(.5); m_conveyor.setDisabled(false); m_conveyor.setReversed(false);},
                () -> {m_intake.runIntake(0); new WaitCommand(4); m_conveyor.setDisabled(true);},
                m_intake
    ));

    new Trigger(OIConstants.DRIVE_GAMEPAD::getBButton).whileTrue(
            new StartEndCommand(
                    () -> {m_intake.runIntake(-.5); m_conveyor.setDisabled(false); m_conveyor.setReversed(true);},
                    () -> {m_intake.runIntake(0); new WaitCommand(4); m_conveyor.setDisabled(true); m_conveyor.setReversed(false);},
                    m_intake
            ));
